...
|--------|------|------|
| `GET` | `/health` | 헬스체크 |
| `POST` | `/api/orders` | 의뢰서 이미지 업로드 (multipart: image, lab_id) — 201 응답 직후 BackgroundTasks 로 OCR→라우팅→스코어링 자동 실행 |
| `GET` | `/api/orders/{order_id}` | 의뢰서 상세 (필드 4종 + presigned 이미지 URL) |
| `PATCH` | `/api/orders/{order_id}/confirm` | (레거시) 일괄 확정 |
| `POST` | `/api/orders/{order_id}/retry-ocr` | OCR 실패 건 수동 재시도 |
| `GET` | `/api/v1/orders/{order_id}/status` | 상태 폴링 전용 경량 엔드포인트 (프론트 TanStack Query) |
| `GET` | `/api/v1/review/queue` | HITL 검토 큐 (needs_review, 최저 신뢰도 순, 페이지네이션) |
| `GET` | `/api/v1/review/{order_id}` | 검토 상세 (필드 envelope + presigned 이미지 URL) |
| `PATCH` | `/api/v1/review/{order_id}/fields/{field_key}` | 인라인 필드 수정 (타입별 검증, 422 구조화 오류) |
| `POST` | `/api/v1/review/{order_id}/confirm` | HITL 확정 + training_labels 일괄 적재 (재호출 409) |
| `GET` | `/api/v1/review/accuracy/fields` | 필드별 자동 정확도 집계 |
...
| `API_AUTH_TOKEN` | Phase 1 단일 베어러 토큰 (빈 값 = 인증 비활성, 로컬 개발) |
| `OCR_PROVIDER` | `clova`(기본) \| `mock` — 로컬 개발은 mock |
| `STORAGE_BACKEND` | `r2`(기본) \| `local` — local 은 `/local-files` 정적 서빙 |
| `CACHE_BACKEND` | `redis`(기본) \| `memory` |
| `PUBLIC_BASE_URL` | local 스토리지 URL 생성용 API 외부 주소 |
...
- **QStash 큐** — 현재 업로드 후 파이프라인은 FastAPI BackgroundTasks 로 실행 (Phase 2 에서 QStash 전환)
- **Type A/Shade 이미지 감지 파이프라인 연결** — 감지 모듈은 구현됐으나 업로드 파이프라인의 라우팅 단계는 텍스트 기반(Type B/C)만 수행
- **Type C LLM 구조화 파이프라인 연결** — LLM 승급 체인은 구현됐으나 라우팅 단계 미연결
- **이미지 해시 캐시 적재** — 캐시 조회는 있으나 OCR 결과 캐시 저장 로직 미연결 (현재 항상 OCR 실행)
- **RBAC 세분화** — Phase 1 은 단일 베어러 토큰 (`API_AUTH_TOKEN`)
- **LLM 모델명 확정** — 배포 전 OpenAI 라인업 확인 필요
- **파일럿 튜닝** — `marking_density_marked`, `shade_mark_ratio` 등 이미지 처리 파라미터 실데이터로 조정 필요

> HITL UI/API + 학습셋 적재 + 프론트-백엔드 연동(업로드→폴링→검토→확정)은 완료됨 (12절 로컬 실행 참고).

---

## 12. 개발 환경 세팅

### 백엔드 (운영 구성: NEON/R2/Redis/CLOVA)
...
### 로컬 풀스택 연동 (외부 의존 0 — mock OCR + sqlite + 로컬 스토리지)
...
cd backend
uv run python -m scripts.dev_bootstrap   # 테이블 생성 + Lab(id=1) 시드
uv run uvicorn app.main:app --port 8000

# 프론트 (별도 터미널) — .env.local 은 .env.local.example 참고
cd frontend
npm install
